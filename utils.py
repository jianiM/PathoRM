...
def Sampling_NegativeSamples(negasample_list,num):
    sample_index = np.random.choice(negasample_list,num)
    nega_sample_index = torch.tensor(sample_index)
    return nega_sample_index
...
def get_drug_dissimmat(drug_affinity_matrix,topk):
    ...
    for i in range(drug_num):
        ...
        for j in range(topk):
            drug_id_list[j] = int(sorted_result[j][0])       
