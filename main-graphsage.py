...
if __name__=="__main__":
    ...
    feature concate 
    """
    site_feature_path = "/home/amber/mulga/extracting_site_semantic_features/rna_embeddings.pt"
    ...
    for fold_int in range(n_splits):
        ...
        # 初始化 GraphSAGE 模型
        graphsage_model = GraphSAGE_autoencoder(input_size=768, hidden_size=hgcn_dim, output_size=hgcn_dim, train_W=train_W, dropout=dropout).to(device)
        graphsage_optimizer = torch.optim.Adam(list(graphsage_model.parameters()), lr=lr)
        fgm = FGM_GraphSAGE(graphsage_model)

        graphsage_model.train()

        for epoch in range(epoch_num):                             
           
            A_hat = graphsage_model(H, G, drug_num, target_num)                    
            ...
            A_hat_adv = graphsage_model(H, G, drug_num, target_num)
