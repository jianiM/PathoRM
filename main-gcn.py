...
from utils import * #Construct_G,Construct_H,one_hot_tensor,Sampling_NegativeSamples,loss_function
from data_reading import read_data
from models.gcn import xavier_init, GraphConvolution, GCN_decoder
import torch 
...
def parse_args():
    ...
    #parser.add_argument('-k', '--k_topofallfeature', type=int, nargs='?', default=1000,help='negative sample')    
...
if __name__=="__main__":
    ...
    #print("drug_dissimmat:",type(drug_dissimmat))
    ...
    disease_feature_path = "/home/amber/mulga/extracting_disease_semantic_embeddings/disease_embeddings_dict.pt" 
    disease_mat = disease_feature_concate(disease_feature_path)
    print("disease_mat:",disease_mat.shape)
    ...
    for fold_int in range(n_splits):
        ...
        gcn_model = GCN_decoder(in_dim=768,hgcn_dim=hgcn_dim,train_W = train_W,dropout=dropout).to(device)    
        # choose the optimizer
        gcn_optimizer = torch.optim.Adam(list(gcn_model.parameters()),lr=lr)                

        fgm = FGM_GCN(gcn_model)
        gcn_model.train()       
        ...
        for epoch in range(epoch_num):                             
            ...
            A_hat_adv = gcn_model(H, G, drug_num, target_num)
